...
- **`aj.classical.theta_functions`:** Riemann theta with characteristic (`riemann_theta_function`), zero-char wrappers `riemann_theta`, `grad_riemann_theta`, `log_theta`, `grad_log_theta`. Uses the lattice-sum implementation (numpy/torch).
...
- **`notebooks/spiral_aj_classification.ipynb`:** 2D spiral classification (like TensorFlow Playground spiral dataset). Trains three models: Forward AJ (genus 2, (x,y) → AJ coords → linear head), Inverse AJ ((x,y) → MLP → u → InverseAbelJacobiNetwork → linear head), ReLU MLP (2→8→8→2). Uses `make_spiral`, small forward tables built in-notebook, toy InverseAbelJacobiNetwork setup (Omega_init, K_init, log_sigma_fun). Run from repo root or `notebooks/`; no external data required.
